...
const isOwner = [`${setting.ownerNumber}`,"08891753410@s.whatsapp.net","6285706429182@s.whatsapp.net"].includes(sender) ? true : false
...
if (satu == "nickname"){ return db_user[x1].nick}
...
if (chats.length === 0) return;
data_deposit.data.trannss = (chats)
let ii = await fetchJson(`https://api.pinostore.my.id/api/short/tiny?apikey=FreeKey&url=${data_deposit.data.trannss}`)
if (ii.status == false) return reply('url tidak valid\nsilahkan kirim yg benar.')
data_deposit.session = "input_texttttranss";
fs.writeFileSync(PathAuto + sender.split("@")[0] + ".json", JSON.stringify(data_deposit, null, 3));
reply(`*SHORTLINK*
...
var ytmp4 = await fetchJson(`https://api.pinostore.my.id/api/downloader/ytmp4?apikey=FreeKey&url=${data_deposit.data.url_nya}`)
var xx = ytmp4.result
...
if (chats.length === 0) return;
data_deposit.data.url_nya = (chats);
var ytmp3 = await fetchJson(`https://api.pinostore.my.id/api/downloader/ytmp3?apikey=FreeKey&url=${data_deposit.data.url_nya}`)
var xx = ytmp3.result
...
*_Seputar bot & transaksi_*

${strip}Telpon/Spam blokir 🚫${strip}

_Admin 1 : 62 889-1753-410_
_Admin 2 : nothing_
...
const gurbot = '628891753410@s.whatsapp.net'
...
var bg_breaksek = '08891753410@s.whatsapp.net'
conn.sendMessage(bg_breaksek, {text: teks + teks1, mentions:[sender]}, {quoted:msg})
...
axios.get(`https://${hostwhm}:2087/json-api/createacct?api.version=1&username=${usern}&contactemail=ristarivica@gmail.com&plan=${pekeg}&domain=${domain}`, authWhm)
.then(response => {     
...
case 'wallml':{
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
let eek = await fetchJson(`https://pinostore.my.id/database/${command}.json`)
...
var dev = '08891753410@s.whatsapp.net'
